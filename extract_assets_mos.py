...
def main():
    ...
    #)
    print("build or check tools/ n64graphics skyconv mio0 aifc_decode")
    os.system('make -s -C tools/ n64graphics skyconv mio0 aifc_decode') # Cowcat
    print("done")
    ...
    for key in keys:
            ...
            with tempfile.NamedTemporaryFile(prefix="ctl", delete=False) as ctl_file:
                with tempfile.NamedTemporaryFile(prefix="tbl", delete=False) as tbl_file:
                    ...
                    for (asset, pos, size, meta) in assets:
                        ...
                        os.system('python3 tools/disassemble_sound_mos.py %s %s --only-samples %s:%s'%(ctl_file.name, tbl_file.name, asset, str(pos))) # Cowcat
                    try:
                        print("extracting sounds done")
	    ...
	    # Cowcat
            os.system('tools/mio0 -d -o %s baserom.%s.z64 baserom.out'%(str(mio0), lang))
           
            with open("baserom.out", "rb") as f:
                image = f.read()
                f.close()
                os.remove("baserom.out")
            #
                ...
                with tempfile.NamedTemporaryFile(prefix="asset", delete=False) as png_file:
                    ...
                    if asset.startswith("textures/skyboxes/") or asset.startswith("levels/ending/cake"):
                        if asset.startswith("textures/skyboxes/"):
                            imagetype = "sky"
                            os.system('tools/skyconv --type %s --combine %s %s'%(imagetype, png_file.name, asset)) # Cowcat
                        else:
                        ...
                        #subprocess.run(
                         ...
                         #   check=True,
                        #)
                        os.system('tools/n64graphics -e %s -g %s -f %s -w %s -h %s'%(png_file.name, asset, fmt, str(w), str(h))) # Cowcat
            else:
    ...
    output = "\n".join(
        [
            ...
            #str(new_version),
            #*sorted(list(new_assets)), # *sorted fails - Cowcat 
            #"",
        ]
    )
